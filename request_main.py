...
def check_status():
    ...
    if result_code == '000':
        rd.set('msg', '000')
        logger.info("==" * 5 + 'status_check' + "==" * 5)
        logger.info(log_time)
        logger.info(res.text)
    else:
        rd.set('msg', '001')
        logger.info("==" * 5 + "status_fail" + "==" * 5)
        logger.info(log_time)
        logger.info(res.text)
...
def door_open():
    log_time = datetime.datetime.now()
    log_time = log_time.strftime("%Y-%m-%d-%H:%M:%S")
    logger.info("==" * 5 + 'DOOR_OPEN --> CLIENT' + "==" * 5)
...
def admin_open():
    res = requests.post(f'{cf_master_server}manage_door', json={'deviceId': cf_device_id, 'doorStatus': 'O'},
                        verify=False)
    logger.info("==" * 5 + "관리자 OPEN SUCCESS" + "==" * 5)
    logger.info(res.text)
...
def admin_close():
    res = requests.post(f'{cf_master_server}manage_door', json={'deviceId': cf_device_id, 'doorStatus': 'C'},
                        verify=False)
    logger.info("==" * 5 + "관리자 CLOSE SUCCESS" + "==" * 5)
    logger.info(res.text)
    rd.delete('door')
    json_data = json.loads(res.text)
    result_code = str(json_data['resultCode'])
